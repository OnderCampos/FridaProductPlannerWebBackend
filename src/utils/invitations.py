...
def create_invitation(
    ...
    """
    Create a new member-project relationship in Firestore.
    
    Args:
        project_id: Project ID for the invitation
        member_id: Member ID to add to the project
        
    Returns:
        Dictionary with the created relationship data
    """
    try:
        db = FIRESTORE_CLIENT
        member_project_ref = db.collection("member_project")
        
        # Generate a new relationship ID
        new_relation_ref = member_project_ref.document()
        relation_id = new_relation_ref.id
        
        # Create member-project relationship data
        relation_data = {
            "id": relation_id,
        ...
        # Save to Firestore
        new_relation_ref.set(relation_data)
        
        logger.info(f"Created member-project relationship {relation_id} for member {member_id} in project {project_id}")
        
        # Remove internal fields
        relation_data.pop("created_at", None)
        relation_data.pop("updated_at", None)
        
        return relation_data
        
    except Exception as e:
        logger.error(f"Error creating member-project relationship: {str(e)}")
        raise
...
def accept_invitation(token: str, user_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Create team member
        from src.utils.members import create_team_member
